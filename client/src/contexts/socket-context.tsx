...
export function SocketProvider({ children }: SocketProviderProps) {
    ...
    useEffect(() => {
        ...
        socket.on(SOCKET_EVENTS.PLAYER_LEFT, (playerId: string) => {
            setPlayersList((prevPlayers) => prevPlayers.filter((p) => p.id !== playerId))
        })

        socket.on(SOCKET_EVENTS.ROOM_CREATED, (roomId: string) => {
            setRoomId(roomId)
        })
        ...
        socket.on(SOCKET_EVENTS.NEW_ROUND, ({round, country, timeLimit}: {round: number, country: string, timeLimit: number}) => {
            setCurrentRound(round)
        })

        socket.on(SOCKET_EVENTS.END_ROUND, ({correctAnswer, scores}: {correctAnswer: string, scores: {id: string, name: string, score: number}[]}) => {
            // TODO: update players list
        })

        socket.on(SOCKET_EVENTS.GOOD_ANSWER, ({playerId, score}: {playerId: string, score: number}) => {
        ...
        socket.on(SOCKET_EVENTS.BAD_ANSWER, ({playerId}: {playerId: string}) => {
